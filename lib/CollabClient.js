...
function CollabClient(document, editor, connection) {
  this.init = false
  this.shadowCopy = ''
  ...
  connection.on('connect', function() {
    ...
    if(latest) { // reset connection on reconnect
      this.connection.queue.unshift(['edit', this.connection.sent])
      this.connection.sent = this.connection.awaitingAck = false
    }
...
CollabClient.prototype.dispatchForeignEdit = function(edit) {// quite time consuming
  ...
  if(this.connection.sent) {
    console.log('transform new against sent: ', Changeset.unpack(this.connection.sent.diff), incomingCs, '->', incomingCs.transformAgainst(Changeset.unpack(this.connection.sent.diff)))
  ...
  this.connection.queue.getEvents('edit').forEach(function(pendingEdit, i) {
    ...
    if(0 == i && !this.connection.sent) pendingEdit.parent = edit.id
