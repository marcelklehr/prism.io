 ...
 */
Editor.prototype.retainSelection = function(ids, main) {// quite time consuming
  var content = this.getContent()
    , markerStart = ids[0]
    , markerEnd = ids[1]
  ...
  var caret = new cs.text.Changeset(
    new cs.text.Insert(this.$editor[0].innerHTML.length, calcHtmlOffset(this.$editor[0], range.startContainer, range.startOffset), '<span id="'+markerStart+'"></span>', 0),
    new cs.text.Insert(this.$editor[0].innerHTML.length, calcHtmlOffset(this.$editor[0], range.endContainer, range.endOffset), '<span id="'+markerEnd+'"></span>', 0)
  )
